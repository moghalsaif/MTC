...
const DOC_CATEGORIES = ['General', 'Contracts', 'Manuals', 'Insurance', 'Receipts', 'SOPs', 'Safety', 'Other'];
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-[#52525B] font-data text-sm">LOADING DOCUMENTS...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="documentation-page">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-4xl sm:text-5xl font-black text-white tracking-tight" data-testid="documentation-title">
            DOCUMENTATION
          </h1>
      ...
      <div className="flex gap-3">
        ...
        <Select value={filterCategory} onValueChange={setFilterCategory}>
          <SelectTrigger data-testid="doc-category-filter" className="bg-[#18181B] border-[#232328] text-white h-10 w-40 text-xs">
            <SelectValue placeholder="Category" />
          </SelectTrigger>
          <SelectContent className="bg-[#18181B] border-[#232328]">
            <SelectItem value="all">All ({documents.length})</SelectItem>
            {DOC_CATEGORIES.filter(c => categoryCounts[c]).map(cat => (
      ...
      {filteredDocs.length === 0 ? (
        <div className="bg-[#18181B] border border-[#232328] rounded-lg p-12 text-center">
          ...
          <p className="text-[#3F3F46] text-sm">{documents.length === 0 ? 'No documents yet. Upload your first document.' : 'No documents match your filters.'}</p>
        </div>
      ) : (
          ...
          <table className="w-full" data-testid="documents-table">
            <thead>
              <tr className="border-b border-[#232328]">
                ...
                <th className="text-left py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">Category</th>
                <th className="text-left py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">File</th>
                <th className="text-center py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">Size</th>
                <th className="text-left py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">Uploaded By</th>
                <th className="text-left py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">Date</th>
                <th className="text-right py-2.5 px-4 text-[10px] font-bold text-[#52525B] uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={doc.id} data-testid={`doc-row-${doc.id}`} className="border-b border-[#232328] last:border-b-0 hover:bg-[#1C1C1F] transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <Icon size={16} className="text-[#F9982E] shrink-0" />
                        <div>
                          <div className="text-white text-sm font-medium">{doc.name}</div>
                          {doc.description && <div className="text-[10px] text-[#3F3F46] mt-0.5 truncate max-w-[200px]">{doc.description}</div>}
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-[10px] font-data px-2 py-0.5 rounded bg-[#232328] text-[#71717A]">{doc.category}</span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-[#232328] text-[#71717A] hover:text-white transition-colors"
                          title="Download"
                        >
                          <Download size={14} />
                        </button>
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-[#EF4444]/20 text-[#71717A] hover:text-[#EF4444] transition-colors"
                          title="Delete"
                        >
                          <Trash2 size={14} />
                        </button>
      ...
      <Dialog open={uploadDialog} onOpenChange={setUploadDialog}>
        <DialogContent className="bg-[#18181B] border-[#232328] text-white max-w-md" data-testid="upload-doc-dialog">
          <DialogHeader>
            <DialogTitle className="font-heading text-xl font-bold">UPLOAD DOCUMENT</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <Select value={uploadForm.category} onValueChange={(v) => setUploadForm({ ...uploadForm, category: v })}>
                <SelectTrigger data-testid="doc-category-select" className="bg-[#0F0F0F] border-[#232328] text-white h-11">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#18181B] border-[#232328]">
            ...
            <div>
              <Label className="text-white text-sm mb-2 block">File * (Max 25MB)</Label>
              <input
                ref={fileInputRef}
                type="file"
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    if (!uploadForm.name) setUploadForm({ ...uploadForm, name: file.name.replace(/\.[^.]+$/, '') });
                  }
                }}
              ...
              />
              {selectedFile && (
                <p className="text-[10px] text-[#52525B] mt-1 font-data">{selectedFile.name} ({formatSize(selectedFile.size)})</p>
              )}
            </div>
          </div>
          <DialogFooter>
            <Button onClick={() => { setUploadDialog(false); setSelectedFile(null); }} className="bg-transparent border border-[#232328] text-white hover:bg-[#232328] rounded-lg">
              Cancel
            </Button>
            <Button
              onClick={handleUpload}
              disabled={uploading || !selectedFile || !uploadForm.name}
              data-testid="confirm-upload-doc"
